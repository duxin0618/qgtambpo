...
class FlexibleReplayPool(ReplayPool):
    ...
    def random_indices(self, batch_size, limit=0):
        if self._size == 0: return np.arange(0, 0)
        return np.random.randint(0, self._size-limit, batch_size)

    def order_indices(self, batch_size, limit=0):
        if self._size == 0: return np.arange(0, 0)
        if batch_size >= self._size:
            return np.random.permutation(range(self._size-limit))
        else:
            return np.random.permutation(range(batch_size-limit))
    ...
    def random_batch_truncation_trajectory(self, batch_size, observation_keys=None, k=0, **kwargs):
        random_indices = self.random_indices(batch_size, limit=k)
        return self.batch_truncation_trajectory_by_indices(
            random_indices, field_name_filter=observation_keys, k=k, **kwargs)

    def order_batch_truncation_trajectory(self, batch_size, observation_keys=None, k=0, **kwargs):
        order_indices = self.order_indices(batch_size, limit=k)
        return self.batch_truncation_trajectory_by_indices(
            order_indices, field_name_filter=observation_keys, k=k, **kwargs)
    ...
    def batch_truncation_trajectory_by_indices(self, indices, field_name_filter=None, k=0):
        if np.any(indices+k % self._max_size > self.size):
            raise ValueError(
                "Tried to retrieve batch with indices greater than current"
                " size")
        ...
        return {
            field_name: np.asarray([self.fields[field_name][indices[i]:indices[i]+k, ...]
